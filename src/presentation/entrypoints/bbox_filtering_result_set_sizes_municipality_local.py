...
def bbox_filtering_result_set_sizes_municipality_local() -> None:
    """
    Benchmark: bounding-box filter at municipality scale (Trondheim) over the
    buildings shapefile, executed locally via GeoPandas. Downloads the pre-baked
    shapefile copy from blob storage before running the timed area-filter pipeline.
    """
    _download_data()
    _benchmark()
...
@monitor(
    query_id="bbox-filtering-result-set-sizes-municipality-local",
